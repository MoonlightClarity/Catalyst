...
const extensionZip = path.join(root, "release", `catalyst-${pkg.version}.zip`);
const sourceZip = path.join(root, "release", `catalyst-${pkg.version}-source.zip`);
...
if (manifest.browser_specific_settings?.gecko?.id !== "catalyst@moonlightclarity") {
...
if (manifest.homepage_url !== "https://github.com/MoonlightClarity/Catalyst") {
  fail("Firefox homepage URL changed");
}

const csp = manifest.content_security_policy?.extension_pages ?? "";
if (!csp.includes("'wasm-unsafe-eval'")) {
  fail("Firefox extension CSP lost wasm-unsafe-eval required by PDFium");
}
if (/(?:^|[ ;])'unsafe-eval'(?:[ ;]|$)/.test(csp)) {
  fail("ordinary unsafe-eval must not be enabled");
}
...
for (const source of distFiles) {
  ...
  if (!fs.existsSync(target)) {
    fail(`Firefox staging is missing Catalyst file: ${relative}`);
    continue;
  }
  if (sha256(source) !== sha256(target)) {
    fail(`Firefox staging modified Catalyst file: ${relative}`);
  }
}
...
for (const forbidden of ["node_modules", "dist", ".git", "release"]) {
...
if (!process.exitCode) {
  console.log(
    `firefox release verify: PASS — Catalyst ${pkg.version}; ${distFiles.length} shared app files; no extension permissions; source + extension artifacts present`,
  );
}
