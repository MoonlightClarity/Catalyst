...
4. Human-scale analytical Maps are the primary user-facing portrayals over that substrate; one global node-link canvas is not the permanent interaction model.
...
- alpha.2 native acceptance against real source material and nontrivial maps;
- validate that rapid capture is materially faster than database-style node creation;
- validate map decomposition/collapse behavior under increasing density;
- refine typography/spacing only after the new mapping geometry works;
- audit remaining command/library/method surfaces against ADR 0014/0015/0016;
- test monochrome, keyboard, reduced-motion, and dense-workspace behavior;
- decide whether the alpha.2 mapping-derived mark earns promotion;
- document all final symbol/interaction contracts.
...
## 0.6.3-alpha.3 — Working Picture / recognition-first renderer (design frozen, implementation next)
...
### Planned coherent implementation slice

- introduce Issue/Working Picture framing without irreversible research-data migration;
- replace universal text-node portrayal with heterogeneous visual occurrences;
- render saved evidence as recognizable source-region crops where current PDF geometry permits it;
- move full body text entirely to inspection/source contexts;
- make structural branches primary and semantic relationships latent by default;
- implement visibly different overview/working/close semantic portrayals;
- implement inspect vs focus vs open as separate actions;
- add reliable Home/Back return to an analytical place;
- preserve local analyst-authored geography during local edits;
- establish the first production-candidate Catalyst analytical glyph grid/family;
- keep all future provenance/inference/Watch semantics explicitly deferred until the domain model exists.
