...
if (-not $ConfirmSubmission) {
  throw 'AMO submission was not confirmed. Re-run with -ConfirmSubmission only when you intend to upload Catalyst to addons.mozilla.org.'
}
...
if (-not $env:AMO_JWT_SECRET) {
  throw 'AMO_JWT_SECRET is not set. Expose the AMO JWT secret only through the environment; do not store it in Catalyst.'
}
...
try {
  ...
  $sourceArchive = ".\release\catalyst-$($package.version)-source.zip"
  ...
  Write-Host 'Submitting Catalyst to the AMO listed channel...'
  $signArgs = @(
    '--yes',
    'web-ext@10.6.0',
    'sign',
