...
await c('Page.navigate',{url:'http://127.0.0.1:5173/?research=unified-workflow&fixture=tradecraft&shell=reader&run=cross-role-source-region'});await sleep(4500);
...
const state=()=>v(`JSON.parse(localStorage.getItem('catalyst.browser.workspace.v1')||'{}')`);
await selectTitle();
await v(`document.querySelector('[aria-label="Highlight source"]')?.click();true`);await sleep(1400);
const afterHighlight=await state();
const firstId=Object.keys(afterHighlight.annotations||{})[0];
await selectTitle();
await v(`document.querySelector('[aria-label="Save evidence"]')?.click();true`);await sleep(1400);
const afterEvidence=await state();
await selectTitle();
await v(`document.querySelector('[aria-label="Create linked thought"]')?.click();true`);await sleep(1400);
const afterThought=await state();
const ids=Object.keys(afterThought.annotations||{});
const roles=afterThought.annotationRoles?.[firstId]||[];
const link=afterThought.links?.[0];
const result={firstId,afterHighlight:{ids:Object.keys(afterHighlight.annotations||{}),roles:afterHighlight.annotationRoles?.[firstId]},afterEvidence:{ids:Object.keys(afterEvidence.annotations||{}),roles:afterEvidence.annotationRoles?.[firstId]},afterThought:{ids,roles,noteIds:Object.keys(afterThought.notes||{}),link},pass:ids.length===1&&ids[0]===firstId&&roles.includes('highlight')&&roles.includes('excerpt')&&link?.annotationId===firstId};
await v(`([...document.querySelectorAll('[role=tab]')].find(e=>e.textContent.trim().startsWith('Evidence')))?.click();true`);await sleep(500);
result.ui=await v(`(()=>({tabs:[...document.querySelectorAll('[role=tab]')].map(e=>e.textContent.trim()),kinds:[...document.querySelectorAll('.evidence-kind')].map(e=>e.textContent.trim())}))()`);
const shot=await c('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});
fs.writeFileSync('C:/Users/iris/Downloads/Catalyst/tools/research-probes/2026-09-12/artifacts/evidence-cross-role-source-region.png',Buffer.from(shot.result.data,'base64'));
