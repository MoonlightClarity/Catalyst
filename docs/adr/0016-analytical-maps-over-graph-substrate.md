...
Superseded in its primary-surface claim by ADR 0017 for 0.6.3-alpha.3. Its Map/occurrence/structural-branch decisions remain active as lower-level portrayal architecture.

This ADR **partially supersedes ADR 0009**; ADR 0017 in turn supersedes only this ADR's claim that Map is the primary human interface. ADR 0009 remains correct that Catalyst's persistent analytical structure is graph-like and that relationships are first-class. It is superseded only where it made one persistent node-link Graph workspace the primary user-facing mental model.
...
Catalyst retains a graph-like **analytical model** as its computational substrate. In alpha.2 the primary human working surface became a **Map**; ADR 0017 subsequently promotes the recognition-first **Working Picture** above Map while retaining Map as saved, purposeful, human-scale portrayal state over the analytical model.
...
- The alpha.1 `GraphWorkspace` coordinate-plane renderer is retired from the canonical alpha.2 source tree.
- 0.6.3-alpha.2 implements the first `AnalysisMapWorkspace` rather than cosmetically refining alpha.1's coordinate plane.
